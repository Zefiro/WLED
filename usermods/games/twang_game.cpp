...
-------------------------------------------------------------
  ...
  Pin 16: Neopixel Data
  Pin 17: (unused / Dotstar Clock)
  Pin 21: I2C SDA -> MPU650 accelerometer (also connected to 3.3V and GND)
  Pin 22: I2C SCL
  Pin 25: DAC Audio Out
  Pin 26: Twang/Pong Gameselector (switch to GND) (optional)
  Pin 32: Pong Player Left (pushbutton to GND)
  Pin 33: Pong Player Right (pushbutton to GND)
  Pin 34: Analog Joystick X
  Pin 35: Analog Joystick Y
  Pin 12: reserved
  Pin 27: reserved

  Configuration:
    C=252 (LED Count 60-288)
    B=150 (LED Brightness 5-255)
    S=20 (Sound Volume 0-255)
    D=8 (Joystick Deadzone 3-12)
    A=30000 (Attack Sensitivity 20000-35000)
    L=3 (Lives per Level (3-9))

-------------------------------------------------------------
  Changes
  15.08.24
  - for Reset, press only right player button for 7 seconds
  - for recalibration during Twang, press button (for 0.5sec)
  - added configurable offset (adds black leds at the start)
    untested. might not initialize to black. might be wrong for some code paths
  - screensaver starts with an effect showing the full length


-------------------------------------------------------------
 TODO

Idle-Erkennung bei dejustiertem Controller
Twang Game starts on movement
Bugfix: sound only works on first game
Pong: Speed kann langsamer werden, aber base-speed erhöht sich
Mehr Screensaver-Effekte
ESPNow?
...
-------------------------------------------------------------
  ...
  - https://projecthub.arduino.cc/flyingangel/1d-pong-1a0acf
    60 leds, 1m, 10 rounds wins, hit zone = 6 blue pixels
    boost/superboost in last 1/2 pixels of hit zone
    also shows hit zone and score (in the middle) always
    shows the exact hit position in the hit zone shortly in the player color
    win = rainbow on the winners side
  - https://www.vagrearg.org/content/onedpong
    60 leds
    colorful idle animation
    each player has 2 buttons: serve and boost.
    speed is increased with each hit and with "hitting late" and with "pressing boost"
    boosting gives a penalty (reduction of hit zone) and adds visual sparkles
    shows not only a single led, but with a trail
    shows 'lives' as constantly-weakly-lit leds in the middle (red/green) and 5 blue leds on each side as the hit zone
    sound: with buzzer, constantly on each move, loose sound, victory song
    winner serves, ball blinks and only moves when served
    someone else build it and posted to Hack-a-Day: https://www.youtube.com/watch?v=14-b0aKh2IM
  - https://www.instructables.com/Make-Your-Own-1D-Pong-Game/ (by Great Scott)
    40 leds plus uses a separate score display (of 2x 5x3 pixels with ledstrip, displaying digits)
    paddle area = last 5 LEDs, which light up red. warning area = 5 yellow leds. outside = green ball
    Loose: push button too early (before red area) or too late
    looser gets to serve
    win animation
    state engine, 5 states:
    0: bootup (3 leds red on each side)
    1: game starts (with either red or green player)
    2: green-to-red
    3: red-to-green
    4: score
    speed: uses millis() to wait X ms, with x=60 at game start, going down by 3ms for each turn (not looking at hit zone)
  - https://www.youtube.com/watch?v=YpqpQz4mYZk (one game, ends after 3:1)
    1 button each. both press to start (after press, their side lights up green. after both, small intro sequence, then start)
    white ball, gets green near the end (in 'paddle area')
    if pressed too late, opposite half of strip turns green ('scores')
    gets faster the closer to the end it's paddled. Slows down again somehow?
    starts from the middle, always in the same direction
*/
...
#include "Bounce2.h"
#define PONG_DEBOUNCE_INTERVAL_MS 5
Bounce2::Button butGameSelect = Bounce2::Button();
Bounce2::Button butPlayerRight = Bounce2::Button();
Bounce2::Button butPlayerLeft = Bounce2::Button();
...
Boss boss = Boss();
IdleAnimation idle(leds);
Pong pong(leds, butPlayerRight, butPlayerLeft);
...
int16_t calibrate_a = INT16_MIN;
int16_t calibrate_g = INT16_MIN;
...
void setup() {
  ...
  butGameSelect.attach (PONG_GAMESELECT_PIN, INPUT_PULLUP);
  butGameSelect.interval(PONG_DEBOUNCE_INTERVAL_MS);
  butGameSelect.setPressedState(HIGH); 
  
  butPlayerRight.attach (PONG_PLAYER_RIGHT_PIN, INPUT_PULLUP);
  butPlayerRight.interval(PONG_DEBOUNCE_INTERVAL_MS);
  butPlayerRight.setPressedState(LOW); 
  
  butPlayerLeft.attach (PONG_PLAYER_LEFT_PIN, INPUT_PULLUP);
  butPlayerLeft.interval(PONG_DEBOUNCE_INTERVAL_MS);
  butPlayerLeft.setPressedState(LOW); 
...
void loop() {
  ...
  if (redraw) {
    // check inputs (same frequency as game logic, otherwise we miss keypresses)
    getInput();
    butGameSelect.update();
    butPlayerRight.update();
    butPlayerLeft.update();

    // "Reset"
    if (globalState != RESETTING && butPlayerRight.isPressed() && butPlayerRight.currentDuration() > RESET_BUTTON_DURATION_MS) {
      changeGlobalState(RESETTING);
    } else if (globalState == RESETTING && !butPlayerRight.isPressed() && butPlayerLeft.isPressed()) {
      // directly go to Pong from Reset if secondary button is pressed after reset
      changeGlobalState(PONG);
    } else if (globalState == RESETTING && !butPlayerRight.isPressed() && !butPlayerLeft.isPressed()) {
      changeGlobalState(IDLE);
    }

    // Timeout for PONG, same logic as for TWANG. Would like to put it into class Pong, but how to change global game state from there?
    if (globalState == PONG) {
      if (butPlayerRight.changed() || butPlayerLeft.changed()) {
        lastInputTime = mm;
      } else if (lastInputTime+TIMEOUT < mm) {
        changeGlobalState(IDLE);
      }
    }

    // Recalibrate Twang on button-press
    if (globalState == TWANG && butPlayerRight.isPressed() && butPlayerRight.currentDuration() > RECALIBRATE_BUTTON_DURATION_MS) {
      calibrate_a = INT16_MIN;
    }
  
    bool resetGame = butGameSelect.changed() || (globalState == IDLE && (butPlayerRight.pressed() || butPlayerLeft.pressed()));
    
    if (resetGame) {
      if ((butGameSelect.changed() && butGameSelect.isPressed()) || (globalState == IDLE && butPlayerRight.pressed())) {
        changeGlobalState(TWANG);
      } else {
        changeGlobalState(PONG);
      }
    }
  ...
  switch(globalState) {
    case TWANG: twangLoop(redraw); break;
    case PONG: pong.loop(redraw); break;
    case RESETTING: ResetLoop(redraw); break;
    default: idle.loop(redraw);
  }

  if (redraw) FastLEDshowESP32();  
}

void changeGlobalState(enum globalstates newState) {
  if (globalState != newState) {
    switch (newState) {
      case TWANG: {
        Serial.println("Game changed to TWANG");
        calibrate_a = INT16_MIN; // reset calibration on start of Twang
        twangResetGame();
      } break;
      case PONG: {
        Serial.println("Game changed to PONG");
        pong.resetGame();
      } break;
      case RESETTING: {
      ...
      case IDLE: {
        calibrate_a = INT16_MIN; // reset calibration when entering idle mode
        Serial.println("Idling...");
      } break;
      default: {
        Serial.printf("Unknown new globalstate %d, idling instead", newState);
        newState = IDLE;
      } break;
    }
...
void ResetLoop(bool redraw) {
  if (!redraw) return;
  for(uint16_t x = 0; x < user_settings.led_offset; x++) {
    leds[x] = CRGB(0, 0, 0);
  }
  for(uint16_t x = 0; x < user_settings.led_count; x++) {
    leds[x+user_settings.led_offset] = CRGB(50, 0, 0);
  }
}

void twangLoop(bool redraw) {
  if (!redraw) return;
  if (stage == PLAY) {
    if (attacking) {
      SFXattacking();
  ...
  if (abs(joystickTilt) > user_settings.joystick_deadzone) {
    ...
    // detect decalibrated idling
    if (abs(joystickTilt - lastJoystickTilt) > user_settings.joystick_deadzone) {
      lastJoystickTilt = joystickTilt;
      lastInputTime_Decalibrated = mm;
    } else if (mm - lastInputTime_Decalibrated > AUTO_RECALIBRATE_DURATION_MS) {
      // recalibrate to current value, if it keeps the same, the screensaver will kick in a bit later
      calibrate_a = INT16_MIN;
    }
  } else if (lastInputTime+TIMEOUT < mm) {
    stage = SCREENSAVER;
  ...
  if (stage == SCREENSAVER) {  // shouldn't happen anymore
    Serial.println("!!!!!!!! unexpected stage=Screensaver !!!!!!!!");
    idle.loop(true);
...
void tickStartup(long mm)
{
  ...
  if(stageStartTime+STARTUP_WIPEUP_DUR > mm) // fill to the top with green
  {
    ...
    for(int i = 0; i<= n; i++){     
      leds[i+user_settings.led_offset] = CRGB(0, 255, 0);
    }   
  }
  else if(stageStartTime+STARTUP_SPARKLE_DUR > mm) // sparkle the full green bar    
  {
    for(int i = 0; i< user_settings.led_count; i++){    
      if(random8(30) < 28)
        leds[i+user_settings.led_offset] = CRGB(0, 255, 0);  // most are green
      else {
        int flicker = random8(250);
        leds[i+user_settings.led_offset] = CRGB(flicker, 150, flicker); // some flicker brighter
      }
    }
  ...
  else if (stageStartTime+STARTUP_FADE_DUR > mm) // fade it out to bottom
  {
    ...
    // for(int i = 0; i<= n; i++){    
      ...
      // leds[i+user_settings.led_offset] = CRGB(0, brightness, 0);
    // }
...
void tickBoss(){
	...
	if(boss.Alive()){
		...
		for(int i = getLED(boss._pos-BOSS_WIDTH/2); i<=getLED(boss._pos+BOSS_WIDTH/2); i++){
			leds[i+user_settings.led_offset] = CRGB::DarkRed;
			leds[i+user_settings.led_offset] %= 100;
		}
		// CHECK COLLISION
...
void tickComplete(long mm)  // the boss is dead
{
  ...
  if(stageStartTime+500 > mm){
    ...
    for(int i = user_settings.led_count; i>= n; i--){
      ...
      leds[i+user_settings.led_offset].setHSV(brightness, 255, 50);
    }
...
void tickBossKilled(long mm) // boss funeral
{
	...
	if(stageStartTime+6500 > mm){
		...
		if( random8() < 200) {  // add glitter
			leds[ random16(user_settings.led_count)+user_settings.led_offset ] += CRGB::White;
		}
		SFXbosskilled();
...
void tickDie(long mm) { // a short bright explosion...particles persist after it.
	...
	if(stageStartTime+duration > mm) {// Spread red from player position up and down the width
		...
		// fill up
		int n = _max(map(((mm-stageStartTime)), 0, duration, getLED(playerPosition), getLED(playerPosition)+width), 0); // TODO Zefiro: shouldn't this be min(..., user_settings.led_count) ?
...
void tickGameover(long mm) {
  ...
  else if(stageStartTime+GAMEOVER_FADE_DURATION > mm)  // fade down to bottom and fade brightness
  {
    ...
    for(int i = 0; i<= n; i++){
      leds[i+user_settings.led_offset] = CRGB(brightness, 0, 0);
    }
    SFXcomplete();
  }
...
void tickWin(long mm) {
  ...
  if(stageStartTime+WIN_FILL_DURATION > mm){
    ...
    leds[0+user_settings.led_offset] = CRGB(0, user_settings.led_brightness, 0);
  }else{        
    nextLevel();
  }
}
...
void drawLives()
{
  ...
  { 
      for (int j=0; j<4; j++)
      {
        leds[pos++ +user_settings.led_offset] = CRGB(0, 255, 0);
        FastLED.show();       
      }
      leds[pos++ +user_settings.led_offset] = CRGB(0, 0, 0);      
      delay(20);
  }
  FastLED.show();
  delay(400);
...
// ----------- JOYSTICK ------------
// ---------------------------------
bool joystick_wobble_mode = false;
void getInput(){
...
// -----------------------------------
// Zefiro: using an analog joystick
#ifdef USE_ANALOG_JOYSTICK
    // read analog input in MilliVols. According to the documentation this is 'calibrated', so assumingly more linear than the ESP raw value
    uint16_t jxv = analogReadMilliVolts(ANALOG_JOYSTICK_X_PIN);
    uint16_t jyv = analogReadMilliVolts(ANALOG_JOYSTICK_Y_PIN);

#define ANALOG_JOYSTICK_MINVALUE_X 142
#define ANALOG_JOYSTICK_MAXVALUE_X 3147
#define ANALOG_JOYSTICK_MINVALUE_Y 142
#define ANALOG_JOYSTICK_MAXVALUE_Y 3147
  ax = 166 * map_constrain(jxv, ANALOG_JOYSTICK_MINVALUE_X, ANALOG_JOYSTICK_MAXVALUE_X, -90, 90);
  gx = abs(map_constrain(jyv, ANALOG_JOYSTICK_MINVALUE_Y, ANALOG_JOYSTICK_MAXVALUE_Y, -5, 5));
  if (gx > 2) {
    if (!joystick_wobble_mode) {
      gx = user_settings.attack_threshold;
      joystick_wobble_mode = true;
    } else {
      // ignored - go back to center to re-activate wobbling
    }
  } else {
    joystick_wobble_mode = false;
  }
#else // ----------------------------------- END using an analog joystick
    ...
    // TODO use real math here (Quaternions) to get an actual angle
    if (calibrate_a == INT16_MIN) {
      // if calibration value is not yet set, use the current value
      calibrate_a = a;
      calibrate_g = g;
    } else {
      a = a - calibrate_a;
    ...
    joystickWobble = abs(MPUWobbleSamples.getHighest());
		...
		#ifdef JOYSTICK_DEBUG
      Serial.printf("tilt: %6d -- wobble: %6d\n", joystickTilt, joystickWobble);
		#endif
